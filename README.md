Drupal/PostgreSQL development with Docker
=========================================

Quick and easy to use Docker container for your *local Drupal development*. It contains a LAPP *(Linux/Apache/PHP/PostgreSQL)* stack and an SSH server, 
along with an up to date version of Drush. It is based on [Debian Jessie](https://wiki.debian.org/DebianJessie). 
It is based on [docker-drupal](https://github.com/wadmiraal/docker-drupal) container developed by [@waadmiral](https://github.com/wadmiraal), 
modifying it for use of PostgreSQL as database server
...
Clone the repository locally and build it:
	...
	git clone https://github.com/paudan/docker-drupal-pg.git
	cd docker-drupal-pg
...
You can specify the specific Drupal version you want, like `7.41` or `8.0.0`. 
...
Here's an example just running the container and forwarding `localhost:8080` and `localhost:8022` to the container:
	...
	docker run -d -p 8080:80 -p 8022:22 -t paudan/drupal

If you want to run in HTTPS, you can use:

        docker run -d -p 8443:443 -p 8022:22 -t paudan/drupal
...
Here's an example running the container, forwarding port `8080` like before, but also mounting Drupal's `sites/all/modules/custom/` folder to my local `modules/` folder. I can then start writing code on my local machine, directly in this folder, and it will be available inside the container:
	...
	docker run -d -p 8080:80 -v `pwd`/modules:/var/www/sites/all/modules/custom -t paudan/drupal
...
You can now call something like:
	...
	drupal --target=docker.paudan_drupal module:download ctools 8.x-3.0-alpha19
...
Example:
	...
	docker run -it --rm -e BLACKFIREIO_SERVER_ID="[your id here]" -e BLACKFIREIO_SERVER_TOKEN="[your token here]" -p 8022:22 -p 8080:80 paudan/drupal
